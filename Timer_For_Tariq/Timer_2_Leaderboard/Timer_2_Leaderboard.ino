...
unsigned int leaderboardArray2[ARRAY_LENGTH] = {0};
...
char leaderboardChar2[ARRAY_LENGTH] = {' ', ' ', ' ', ' ', ' ', ' '};
...
void writeLoop2(unsigned int arrayToWrite[], char charArrayToWrite[])
{
  int controlIndex = 0;
  for (int writeLoopControl = 120; writeLoopControl < 180; writeLoopControl = writeLoopControl + 10)
  {
    myMem.put(writeLoopControl, arrayToWrite[controlIndex]);
    controlIndex++;
    delay(5);
  ...
  controlIndex = 0;
  for (int charWriteLoopControl = 180; charWriteLoopControl < 240; charWriteLoopControl = charWriteLoopControl + 10)
  {
    myMem.put(charWriteLoopControl, charArrayToWrite[controlIndex]);
    controlIndex++;
    delay(5); 
...
void readLoop2(unsigned int arrayToRead[], char charArrayToRead[])
{
  int indexControl = 0;
  for (int loopControl = 120; loopControl < 180; loopControl = loopControl + 10)
  {
    myMem.get(loopControl, arrayToRead[indexControl]);
    indexControl++;
  }
  ...
  indexControl = 0;
  for (int charLoopControl = 180; charLoopControl < 240; charLoopControl = charLoopControl + 10)
  {
    myMem.get(charLoopControl, charArrayToRead[indexControl]);
    indexControl++; 
  }
...
void setup() 
{
  ...
  readLoop2(leaderboardArray2, leaderboardChar2);
  secondConverter(leaderboardArray[0], timeArray);
}

void loop() 
{
  if(leaderboardSwitch == 0)
  {
    if(digitalRead(buttonAdd) == PUSH)
    {
      //switch to the other leaderboard
      leaderboardSwitch = 1;
  
      //reset index
      index = 0;
      
      //display to let user know switch is occuring
      lcd.setCursor(0, 0);
      lcd.print("SWITCHING TO <--");
      lcd.setCursor(0, 1);
      lcd.print("  HEADING AWAY  ");
  
      //delay to read message and prevent double pushing
      delay(4000);
    }
  }

  if(leaderboardSwitch == 1)
  {
  ...
  //Leaderboard 2 depending on input
  else if(leaderboardSwitch == 1)
  {
    secondConverter(leaderboardArray2[index], timeArray);
    ...
    lcd.print("                ");  
    lcd.setCursor(0, 1);
    lcd.print(leaderboardChar2[index]);
    lcd.setCursor(1, 1);
    lcd.print(")  ");
    lcd.setCursor(4, 1);
    lcd.print(timeArray[0]);
    ...
    if (digitalRead(buttonStart) == PUSH)
    {
      ...
      quickFix(leaderboardArray2);
      
      //if time recorded does not belong in top 5 (shortest) times
      if (seconds >= leaderboardArray2[4])
      {
        //display message stating it was not in the top 5
        lcd.setCursor(0, 0);
        lcd.print("                "); 
        ...
        quickFixUndo(leaderboardArray2);
      }
        ...
        { 
          ...
          { 
            ...
            while(digitalRead(buttonSelect) == NOPUSH)
            {
              ...
              leaderboardChar2[5] = letterArray[charIndex];
            } 
            
            
            sortArray(leaderboardArray2, leaderboardChar2);
            
            //undo to make the display look better
            quickFixUndo(leaderboardArray2);
            
            whileVar = 1;
            writeLoop2(leaderboardArray2, leaderboardChar2);
            delay(500);
          }
    ...
    else if (digitalRead(buttonSelect) == PUSH)
    {
      ...
      //must be outside of loop to allow up and down button to function
      charIndex = 0;
      while ((letterArray[charIndex] != leaderboardChar2[index]) && (leaderboardChar2[index] != 'x'))
      {
        charIndex++;
      }
      ...
      { 
        ...
        {
          //line to set currently selected time to 0
          leaderboardArray2[index] = 0;
          leaderboardChar2[index] = 'x';
          leaderboardArray2[5] = 0;
          leaderboardChar2[5] = 'x';
        ...
        {
          leaderboardChar2[index] = letterArray[charIndex];
          writeLoop2(leaderboardArray2, leaderboardChar2);
